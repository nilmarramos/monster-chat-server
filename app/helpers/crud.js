module.exports = (app) => {
  ...
  return {
    createUser: (Model, res) =>
      Model.save()
        .then(successOrError.success(res, Return.successCreateUser))
        .catch(successOrError.error(res)),

    create: (Model, res) =>
      Model.save()
        .then(successOrError.success(res, Return.success))
        .catch(successOrError.error(res)),

    listAll: (Model, query, mod, res) =>
      Model.paginate(query, mod)
        .then(successOrError.success(res, Return.success))
        .catch(successOrError.error(res)),
    ...
    update: (Model, query, mod, res) =>
      Model.update(query, mod)
        .then(successOrError.success(res, Return.returnUpdateSuccess))
        .catch(successOrError.error(res)),

    findOneAndUpdate: (Model, query, mod, res, options) =>
      Model.findOneAndUpdate(query, mod, options)
        .then(successOrError.success(res, Return.returnUpdateSuccess))
        .catch(successOrError.error(res)),

    delete: (Model, query, opt, res) =>
      Model.findByIdAndRemove(query, opt)
        .then(successOrError.success(res, Return.success))
        .catch(successOrError.error(res)),

    findByIdAndUpdate: (Model, query, options) =>
      Model.findByIdAndUpdate(query, options)
        .then(successOrError.success(res, Return.success))
        .catch(successOrError.error(res)),
